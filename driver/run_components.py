...
def run_search(args):
    ...
    if "time" == args.profile:
        args.search_options = '--tool=callgrind --dump-instr=yes --collect-jumps=yes --callgrind-out-file=callgrind.out'.split(' ') + [search] + args.search_options
        search = "valgrind"
    elif "memory" == args.profile:
        args.search_options = '--leak-check=full --track-origins=yes --log-file=memory-info.out'.split(' ') + [search] + args.search_options
        search = "valgrind"
