...
def parse_effects(alist, result, type_dict, predicate_dict, outcome_labels, objmap):
    ...
    normalized = tmp_effect.normalize(objmap)
    cost_eff, rest_effect = normalized.extract_cost()
    single_effect_result = []
    outcome_index = 0
    if isinstance(rest_effect, pddl.OneofEffect):
        for eff in rest_effect.effects:
            outcome_index += 1
            add_effect(eff, single_effect_result)
            result.append(single_effect_result)
            if isinstance(eff, pddl.LabeledOutcomeEffect):
                outcome_labels.append(eff.outcome_label)
            else:
                outcome_labels.append(str(outcome_index))
            single_effect_result = []
    else:
        add_effect(rest_effect, single_effect_result)
        result.append(single_effect_result)
    if cost_eff:
...
def add_effect(tmp_effect, result):
    ...
    if isinstance(tmp_effect, pddl.ConjunctiveEffect):
        ...
        return
    elif isinstance(tmp_effect, pddl.LabeledOutcomeEffect):
        add_effect(tmp_effect.effect, result)
        return
    else:
        parameters = []
...
def parse_effect(alist, type_dict, predicate_dict):
    ...
    if tag == "and":
        ...
        if len(alist) == 2:
            print("WARNING: Single outcome in oneof effect.")
        assert len(alist) >= 2
        return pddl.OneofEffect(
            [parse_effect(eff, type_dict, predicate_dict) for eff in alist[1:]])
    elif tag == "labeled-oneof":
        if len(alist) == 3:
            print("WARNING: Single outcome in labeled-oneof effect.")
        assert len(alist) >= 3
        return pddl.LabeledOneofEffect(alist[1],
            [parse_effect(eff, type_dict, predicate_dict) for eff in alist[2:]])
    elif tag == "outcome":
        assert len(alist) == 3
        return pddl.LabeledOutcomeEffect(alist[1], parse_effect(alist[2], type_dict, predicate_dict))
    else:
...
def parse_actions(alist, type_dict, predicate_dict, objmap):
    iterator = iter(alist)
    ...
    if effect_list:
        try:
            cost = parse_effects(
                effect_list, effs, type_dict, predicate_dict, outcome_labels, objmap)
    ...
    if effs:
        if outcome_labels:
            names = ["%s_DETDUP_%s" % (name, outcome_labels[i]) for i in range(len(effs))]
        else:
            assert len(effs) == 1
            names = [name]
        return [pddl.Action(names[i], parameters, len(parameters), precondition, effs[i], cost)
...
def parse_task(domain_pddl, task_pddl):
    domain_name, domain_requirements, types, type_dict, constants, predicates, predicate_dict, functions, actions, axioms \
                 = parse_domain_pddl(domain_pddl, extract_objects(task_pddl))
...
def parse_domain_pddl(domain_pddl, objlist):
    ...
    for entry in entries:
        if entry[0] == ":derived":
            ...
            actions = parse_actions(entry, type_dict, predicate_dict, objlist2map(objlist+constants, type_dict))
            the_actions.extend(actions)
    yield the_actions
    yield the_axioms

def objlist2map(objlist, type_dict):
    objmap = {t: [] for t in type_dict.keys()}
    for obj in objlist:
        typ = type_dict[obj.type_name]
        for t in [typ.name] + typ.supertype_names:
            objmap[t].append(obj)
    return objmap

def extract_objects(task_pddl):
    iterator = iter(task_pddl)
    tag = next(iterator)
    while tag:
        if tag[0] == ":objects":
            return parse_typed_list(tag[1:])
        else:
            tag = next(iterator, None)
    return []
