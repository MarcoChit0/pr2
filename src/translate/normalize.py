...
from pddl_parser.parsing_functions import set_supertypes
...
# [1.1] Remove universal quantifications from conditions.
#
...
def remove_universal_quantifiers_without_axioms(task):
    ...
    set_supertypes(task.types)
    name2type = dict((t.name, t) for t in task.types)
    ...
    def recurse(condition):
        ...
        if isinstance(condition, pddl.UniversalCondition):
            ...
            for param in condition.parameters:
                type_lists.append(filter(lambda obj: obj.type_name in [param.type_name] + name2type[param.type_name].supertype_names, task.objects))
    ...
    for proxy in tuple(all_conditions(task)):
        ...
        if isinstance(proxy.condition, pddl.Disjunction):
            ...
            for part in proxy.condition.parts:
                ...
                new_proxy.owner.name = new_proxy.owner.name + f"__or_variant{index}"
                new_proxy.set(part)
