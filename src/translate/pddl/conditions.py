...
class Conjunction(JunctorCondition):
    ...
    def clone_and_instantiate(self, var_mapping):
        return Conjunction([part.clone_and_instantiate(var_mapping)
                            for part in self.parts])
    def negate(self):
...
class Disjunction(JunctorCondition):
    ...
    def clone_and_instantiate(self, var_mapping):
        return Disjunction([part.clone_and_instantiate(var_mapping)
                            for part in self.parts])
    def negate(self):
...
class NegatedAtom(Literal):
    ...
    def clone_and_instantiate(self, var_mapping):
        args = [var_mapping.get(arg, arg) for arg in self.args]
        return NegatedAtom(self.predicate, args)
    def negate(self):
