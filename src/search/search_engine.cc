...
void SearchEngine::search() {
    ...
    if (PRP.deadend.record_online) {
        PRP.deadend.found_online.clear();
        delete PRP.deadend.online_policy;
        PRP.deadend.online_policy = new Policy();
    }

    initialize();
    utils::CountdownTimer timer(max_time);
    while ((status == IN_PROGRESS) && PRP.time.time_left()) {
        status = step();
        if (timer.is_expired()) {
    ...
    if (PRP.time.time_left()) {
        ...
        if (PRP.deadend.record_online &&
            !PRP.weaksearch.limit_states &&
            PRP.deadend.found_online.size() > 0)
                update_deadends(PRP.deadend.found_online);

        if (PRP.logging.verbose && statistics.get_generated() > 2) {
            cout << "Generated " << statistics.get_generated() << " state(s)." << endl;

            if (PRP.deadend.record_online && !PRP.weaksearch.limit_states)
                cout << "Dead ends: " << statistics.get_deadend_states() << " state(s). ("
                     << PRP.deadend.found_online.size() << " recorded)" << endl;
            else
                cout << "Dead ends: " << statistics.get_deadend_states() << " state(s)." << endl;
        }
