...
RelaxationHeuristic::RelaxationHeuristic(const options::Options &opts, bool simp)
    : Heuristic(opts), do_simplification(simp) {
    // Build propositions.
    int prop_id = 0;
    VariablesProxy variables = task_proxy.get_variables();
    propositions.resize(variables.size());
    for (FactProxy fact : variables.get_facts()) {
        unpropositions.push_back(fact.get_name());
