...
SearchStatus LazySearch::step() {
    ...
    if (PRP.logging.weak_search) {
        cout << "\n\n   { Step }\n" << endl;
        cout << "Current State:" << endl;
        current_state.dump_pddl();
        if (current_predecessor_id != StateID::no_state) {
            cout << "\nPred State:" << endl;
            state_registry.lookup_state(current_predecessor_id).dump_pddl();
            cout << "\nOperator: " << current_operator->get_name() << endl;
        }
        cout << "\nAdjusted / Real Cost: " << current_g << " / " << current_real_g << endl;
    }
    ...
    if (node.is_new() || reopen) {
        ...
        if (PRP.weaksearch.limit_states) {
            if (state_count > PRP.weaksearch.max_states)
                return FAILED;
            else
                state_count++;
        }
