...
bool test_goal(const StateInterface &state) {
    if (PRP.solution.incumbent)
        return test_policy(state);

    for (auto varval : g_goal) {
        if (state[varval.first] != varval.second) {
            return false;
        }
    }
    ...
    for (auto varval : g_goal)
        (*gs)[varval.first] = varval.second;
    SolutionStep *grs = new SolutionStep(gs, PRP.solution.incumbent->network, 0, NULL, -1, true, true, true);

    PRP.general.matched_step = grs;

    return true;
}

bool test_policy(const StateInterface &state) {

    SolutionStep * best_step = PRP.solution.incumbent->get_step(state);

    if ((best_step && PRP.weaksearch.stop_on_policy) || (best_step && best_step->is_goal)) {
#ifndef NDEBUG
        if (!best_step->is_goal) {
            PartialState *cur = new PartialState(state);
            assert(!is_forbidden(*cur, best_step->op));
            delete cur;
        }
#endif
        PRP.general.matched_step = best_step;
        return true;
    } else {
        return false;
    }
...
void read_goal(istream &in) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        PRP.localize.original_goal.push_back(make_pair(var, val));
    }
...
void read_everything(istream &in) {
    ...
    g_successor_generator_orig = new successor_generator::SuccessorGenerator(task_proxy);
...
successor_generator::SuccessorGenerator *g_successor_generator_orig; // Renamed so the ops can be pruned based on deadends
DeadendAwareSuccessorGenerator *g_successor_generator;


PRPWrapper PRP; // Holds all of the settings and data for PRP
